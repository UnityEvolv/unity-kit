...
describe("Table", () => {
  describe("table layout", () => {
    it("renders a table from the column definition and rows", () => {
      ...
      expect(
        screen.getByRole("cell", { name: "Boardroom" }),
      ).toBeInTheDocument();
    });
    ...
    it("reads row[key] when a column has no cell renderer and uses cell when it has one", () => {
      ...
      expect(
        screen.getByRole("cell", { name: "Reception (4)" }),
      ).toBeInTheDocument();
    });
    ...
    it("aligns a column to the end on request", () => {
      ...
      expect(screen.getByRole("columnheader", { name: "Seats" })).toHaveClass(
        "text-end",
      );
  ...
  describe("selection", () => {
    ...
    it("reports the full next selection and reflects the controlled one", () => {
      ...
      expect(
        screen.getByRole("checkbox", { name: "Reception" }),
      ).not.toBeChecked();
    ...
    it("names a checkbox from rowLabel when the title is not text", () => {
      ...
      expect(
        screen.getByRole("checkbox", { name: "Select Reception" }),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("card layout", () => {
    ...
    it("selects, activates and shows states the same way as the table", () => {
      ...
      fireEvent.click(
        within(screen.getAllByRole("listitem")[0]).getByRole("definition"),
      );
      expect(onRowClick).toHaveBeenLastCalledWith(rooms[0]);
    });
    ...
    it("shows loading, empty and error states in place of the cards", () => {
      ...
      rerender(
        <Table columns={columns} rows={[]} rowKey={rowKey} layout="cards" />,
      );
      expect(screen.getByText("Nothing to show")).toBeInTheDocument();
      rerender(
        <Table
